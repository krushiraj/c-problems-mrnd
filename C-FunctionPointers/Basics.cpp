...
char * subString(char *str, int a, int b){
	char *str1 = (char *)malloc((b - a + 2)*sizeof(char));
	int k = 0;
	for (int i = a - 1; i < b; i++)
	{
		str1[k] = str[i];
...
{
	char *str2 = (char *)malloc((b - a + 2)*sizeof(char));
	str2 = (*function_ptr)(str, a, b);
	//Invoke function_ptr function here, which should call subString along with a,b and return respective value
	return str2;
}

void task3() {
	char name[8] = { 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'a' };
	invokeSubStr(&subString, name, 2, 4);
	/*char str[8] = { 'a', 'b', 'c', 'd', 'e' };
	int a = 2;
	int b = 3;
	char *(*P)(char *, int, int);
	P = subString;
	invokeSubStr(P, str, a, b);
