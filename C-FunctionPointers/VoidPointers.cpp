...
void typeCastToAllDataTypes(void * long_ptr, void * float_ptr, void * char_ptr, void * str_ptr)
{
	long *c;
	c = (long *)long_ptr;
	printf("value stored long_ptr is %ld\n", *c);
	float *d;
	d = (float*)float_ptr;
	printf("value stored at float_ptr is %ld\n", *d);
	char *e;
	e = (char *)char_ptr;
	printf("values stored at char_ptr is %c\n", *e);
	char *f;
	f = (char*)str_ptr;
	printf("values stored at str_ptr is %s\n", f);
}
...
void task9() {
	...
	void *vb = (void*)&b;
	void *vc = (void *)&c;
	getTwoGreater(va, vb, vc, &compareInt);
