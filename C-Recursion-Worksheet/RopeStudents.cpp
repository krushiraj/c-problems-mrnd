...
void findminmax(struct student *le, int c, int *l, int *min, int *max)
{
	if (le == NULL)
	{
		if (*l < c)
    ...
    findminmax(le->first, c+1, l, min, max);
	findminmax(le->second, c+1, l, min, max);
	findminmax(le->third, c+1, l, min, max);
	return;
}
...
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance)
{
	...
	int l = 0, c = 0,min=1000,max=0;
	findminmax(leaderStudent, c, &l, &min, &max);
	*minBallDistance = min;
	*maxBallDistance = max;
	return;
}
