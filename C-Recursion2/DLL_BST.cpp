...
void identical(struct node_bst *root,int *same)
{
	if (root == NULL)
		return;
	identical(root->left,same);
	if (root->data != head->data)
	{
		*same = 0;
	}
	head = head->next;
	identical(root->right,same);
}

int is_identical(struct node_dll *_head, struct node_bst *root) 
{
